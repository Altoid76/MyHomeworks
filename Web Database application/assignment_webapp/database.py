...
def user_playlists(username):
    ...
    try:
        ...
        SELECT * 
        FROM 
        mediaserver.mediacollection col natural join mediaserver.mediacollectioncontents
        WHERE col.username=%s;
        """
...
def user_podcast_subscriptions(username):
    ...
    try:
        ...
        sql = """
        SELECT * 
        FROM mediaserver.podcast p NATURAL JOIN  mediaserver.subscribed_podcasts sub
        WHERE sub.username=%s;
        """
...
def user_in_progress_items(username):
    ...
    try:
        ...
        sql = """
        SELECT  con.media_id,con.play_count, con.progress,con.lastviewed,item.storage_location
        FROM  mediaserver.usermediaconsumption con natural join   mediaserver.mediaItem item
        WHERE con.username=%s and con.progress<100;
        """
...
def get_alltvshows():
    ...
    try:
        ...
        sql = """
        SELECT 
                ts.*, te.count as count  
            FROM
                mediaserver.TVShow ts, 
                (select 
                    t1.tvshow_id, count(*) as count 
                from 
                    mediaserver.TVShow t1 left outer join mediaserver.TVEpisode te1 on (t1.tvshow_id=te1.tvshow_id) 
                    group by t1.tvshow_id) te 
            where ts.tvshow_id = te.tvshow_id;
        """
...
def get_song(song_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
        SELECT song.song_title,art.artist_name as artists,song.length
        FROM mediaserver.song song natural join mediaserver.song_artists songart inner join
        mediaserver.artist art on(songart.performing_artist_id=art.artist_id)
...
def get_song_metadata(song_id):
    ...
    try:
        ...
        sql = """
        SELECT DISTINCT song.song_title,art.artist_name as artists,md_type_name,song.length,md.md_value
FROM mediaserver.song song join mediaserver.AudioMedia A on (song.song_id=A.media_id) join mediaserver.MediaItemMetaData using(media_id) join mediaserver.MetaData md using(md_id) join mediaserver. MetaDataType using(md_type_id) natural join mediaserver.song_artists songart inner join
mediaserver.artist art on(songart.performing_artist_id=art.artist_id)
WHERE song.song_id=%s;
        """
        r = dictfetchall(cur,sql,(song_id,))
...
def get_podcast(podcast_id):
    ...
    try:
        ...
        sql = """
        select *
        from (mediaserver.Podcast JOIN mediaserver.PodcastMetaData using(podcast_id))JOIN mediaserver.MetaData using(md_id) Natural join mediaserver.MetadataType
        where podcast_id = %s;
...
from mediaserver.PodcastEpisode pe left outer join 
    (mediaserver.PodcastMetaData natural join mediaserver.mediaitemmetadata natural join mediaserver.MetaData natural join mediaserver.MetaDataType ) pemd
    using (media_id)
where pe.media_id=%s;
        """
...
def get_album(album_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
        select Album.album_id, Album.album_title,AlbumMetaData.md_id, MetaData.md_value,MetaDatatype.md_type_name
        from mediaserver.Album join mediaserver.AlbumMetaData using (album_id) join mediaserver.MetaData using (md_id) natural join mediaserver.metadatatype
...
def get_album_songs(album_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
        select Song.song_id, Song.song_title, Artist.artist_name as artists
        from  mediaserver.Album join mediaserver.Album_Songs using (album_id) join mediaserver.Song using(song_id) join mediaserver.Song_Artists using(song_id) join mediaserver.Artist on (performing_artist_id=artist_id)
        where Album.album_id=%s
        order by  Album_Songs.track_num ASC;        
        """
...
def get_album_genres(album_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
        SELECT DISTINCT md.md_value,mdt.md_type_name
        FROM mediaserver.album_songs ns natural join mediaserver.song s inner join mediaserver.mediaitemmetadata mit on(s.song_id=mit.media_id) natural join mediaserver.metadata md natural join
        mediaserver.metadatatype mdt
        where ns.album_id=%s;
        """
...
def get_tvshow(tvshow_id):
    ...
    try:
        ...
        sql = """
        SELECT *
        from mediaserver.TVshow tv join mediaserver.TVShowMetaData mtv using(tvshow_id) join mediaserver.MetaData md using(md_id) join mediaserver.MetaDataType mdt using(md_type_id)
...
def get_all_tvshoweps_for_tvshow(tvshow_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
        SELECT tv.media_id,tv.tvshow_episode_title,tv.season,tv.episode,tv.air_date
        FROM Mediaserver.TVEpisode tv
        WHERE tv.tvshow_id=%s
...
#   Add a new Song
#####################################################
def add_song_to_db(storage_location, description, title, song_length, genre, artistid):
    """
        Get all the matching Movies in your media server
        """
    #########
        ...
        sql = """
                    SELECT 
                        mediaserver.addSong(
                            %s,%s,%s,%s,%s,%s);
                    """
        cur.execute(sql, (storage_location, description, title, song_length, genre, artistid))
        conn.commit()
...
def get_last_movie():
    ...
    conn.close()                    # Close the connection to the db
    return None
def change_password(get_username,old_password,new_password):
    """
        change password
        """
    #########
    ...
    try:
        ...
        sql = """
                    SELECT 
                        mediaserver.change_password(
                            %s,%s,%s);
                    """
        cur.execute(sql, (get_username,old_password,new_password))
        r = cur.fetchone()
        print("return val is:")
        print(r)
        cur.close()  # Close the cursor
