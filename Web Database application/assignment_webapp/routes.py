...
def single_podcast(podcast_id):
    ...
    page['title'] = 'podcast' + podcast_id  # Add the title
...
def single_podcastep(media_id):
    ...
    page['title'] = 'podcastep' + media_id  # Add the title
...
def add_song():
    ...
    if (request.method == 'POST'):
        ...
        # forward to the database to manage insert
        songs = database.add_song_to_db(newdict['song_title'], newdict['song_length'], newdict['storage_location'],
                                        newdict['description'], newdict['song_genre'], newdict['artistid'])
...
def change_password():
    ...
    page['title'] = 'Change Password'  # Add the title
    ...
    if (request.method == 'POST'):
        ...
        if ('user_name' not in request.form):
            newdict['user_name'] = 'No name Value'
        else:
            newdict['user_name'] = request.form['user_name']
            print("We have a value: ", newdict['user_name'])

        if ('old_password' not in request.form):
            newdict['old_password'] = 'No Password Value'
        else:
            newdict['old_password'] = request.form['old_password']
            print("We have a value: ", newdict['old_password'])

        if ('new_password' not in request.form):
            newdict['new_password'] = 'Empty Password Value'
        else:
            newdict['new_password'] = request.form['new_password']
            print("We have a value: ", newdict['new_password'])
        ...
        # forward to the database to manage insert
        new_password = database.change_password(newdict['user_name'], newdict['old_password'], newdict['new_password'])
        return redirect(url_for('login'))
    else:
        return render_template('createitems/changepassword.html',
                               session=session,
                               page=page,
                               user=user_details)